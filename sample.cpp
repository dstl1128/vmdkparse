...
{
    if (vcn < attr->StartVcn || vcn > attr->LastVcn) return FALSE;
    *lcn = 0;
    ULONGLONG base = attr->StartVcn;
    for (PUCHAR run = PUCHAR(Padd(attr, attr->DataRunOffset));
            *run != 0;
            run += RunLength(run)) {
        *lcn += RunLCN(run);
        *count = RunCount(run);
...
ULONG AttributeLength(PATTRIBUTE attr)
{
    return attr->Nonresident == FALSE
        ...
        : ULONG(PNONRESIDENT_ATTRIBUTE(attr)->RealSize);
}
...
VOID ReadAttribute(PATTRIBUTE attr, PVOID buffer)
{
    if (attr->Nonresident == FALSE) {
        ...
        ReadExternalAttribute(nattr, 0, ULONG(nattr->LastVcn) + 1, buffer);
    }
}
...
VOID ReadVCN(PFILE_RECORD_HEADER file, ATTRIBUTE_TYPE type, ULONGLONG vcn, ULONG count, PVOID buffer)
{
    ...
    if (attr == 0 || (vcn < attr->StartVcn || vcn > attr->LastVcn)) {
        // Support for huge files
        PATTRIBUTE attrlist = FindAttribute(file, eAttributeAttributeList, 0);
        //DebugBreak();
        fprintf(stderr, "Error: TODO: Need support for huge file.");
    }
...
VOID ReadFileRecord(ULONG index, PFILE_RECORD_HEADER file)
{
    ...
    ReadVCN(MFT, eAttributeData, vcn, clusters, p);
...
VOID FindDeleted()
{
    PATTRIBUTE attr = FindAttribute(MFT, eAttributeBitmap, 0);
    PUCHAR bitmap = new UCHAR[AttributeLengthAllocated(attr)];
    ReadAttribute(attr, bitmap);
    ULONG n = AttributeLength(FindAttribute(MFT, eAttributeData, 0))
        / BytesPerFileRecord;
    ...
    for (ULONG i = 0; i < n; i++) {
        ...
        if (strncmp((char*)&file->Ntfs.Type,"ELIF",4) == 0 && (file->Flags & 1) == 0) {
            attr = FindAttribute(file, eAttributeFileName, 0);
            if (attr == 0) continue;
...
VOID DumpData(ULONG index, PCSTR filename)
{
    ...
    if (strncmp((char*)&file->Ntfs.Type, "ELIF", 4) != 0) return;
    PATTRIBUTE attr = FindAttribute(file, eAttributeData, 0);
    if (attr == 0) return;
